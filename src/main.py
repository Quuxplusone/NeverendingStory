...
from .worldmodel import create_place, get_place
...
def get_longdesc_input_element(placeholder_text):
    return '<textarea name="longdesc" id="longdesc" cols="75" rows="20" pattern=".{2,9999}" placeholder="%s" autocomplete="off" required></textarea>' % placeholder_text
...
@bottle.post('/visit/<num>')
def visit(num):
    place = get_place(num)
    assert place is not None
    if place.num != num:
        bottle.redirect('/visit/%s' % place.num, 302)
    result = '<html>'
    result += '<head>'
    ...
    result += '</div>'
    result += '</form>'
    result += get_visit_javascript(num)
    result += '</body></html>'
    return result
...
def create(predecessor_num):
    ...
    if predecessor.num != predecessor_num:
        bottle.redirect('/visit/%s' % predecessor.num, 302)
    try:
        ...
        bottle.redirect('/visit/%s' % predecessor, 302)
    successor = create_place(predecessor, how, longdesc)
    bottle.redirect('/visit/%s' % successor, 302)
